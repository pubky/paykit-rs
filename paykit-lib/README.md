...
- `ReceiptId`, `ReceiptDraft`, `Receipt`, `PreparedReceipt`, `ReceiptAccess`, `ReceiptAccessEventMessage`, `ReceiptDecryptionKey`, `prepare_receipt`, `store_prepared_receipt`, `send_receipt_access`, `parse_receipt_access_event_message`, `parse_receipt_access_json`, `decrypt_receipt`, and `ENCRYPTED_RECEIPT_MAX_BYTES` for Encrypted Receipt issuance, access delivery, decryption, and encrypted-document sizing.
