...
- Private payments are latest-state data: when multiple private payment envelopes are queued, `get_private_payments` returns the latest and supersedes older private-payment envelopes. Receipt access is event-like: every `paykit.receipt_access` message must be preserved and returned FIFO by `get_receipt_access` instead of using latest-wins semantics. Future event-like kinds must preserve all messages in send order.
