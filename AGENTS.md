...
- Pubky routing helpers live in `paykit-lib/src/pubky_routing.rs`; they own the identity-wide App Registry path, app-scoped Payment Endpoint paths, and shared private path prefixes. Reuse those helpers and constants instead of hard-coding `/pub/paykit/v0/...` paths.
...
- Public Payment Endpoint reads treat missing files/directories (404/GONE) as `None` or an empty list. Endpoint and Public Contact Marker discovery rely on directory listings; application discovery uses the identity-wide Paykit App Registry.
...
- **Public** Payment Endpoints use `PaymentEndpointPayload` (a UTF-8 `String` wrapper). Each endpoint is stored as a separate file under `/pub/paykit/v0/apps/{app_id}/endpoints/`. `set_payment_endpoint` / `remove_payment_endpoint` operate on `PubkySession`; `get_payment_list` / `get_payment_endpoint` operate on `PublicStorage`.
- Counterparty-specific Payment Lists are shared as Private Payment List messages over `pubky-noise`'s `PubkyNoiseEncryptor`, which manages encryption, file naming, and storage via `send_message`/`receive_message`. The Private Payment List plaintext is versioned JSON (`version`, `kind`, `app_id`, and `payment_endpoints`). Payment References belong in Payment Requests, Payment Proofs, and Receipts, not in endpoint publication. The `write_path` and `read_path` (asymmetric folder prefixes derived per-counterparty identity pair via `pubky_noise::path_derivation::derive_asymmetric_paths`) are set during `initiate_encrypted_link` / `accept_encrypted_link`; pubky-noise manages individual file slots within those folders using a counter-based scheme.
...
- Private Payment Lists use Latest-State Message semantics per Paykit App at the SDK/runtime layer: the latest valid list from one App supersedes older list messages from that same App without superseding other Apps' lists. Receipt Access and Payment Request protocol messages (`paykit.payment_request`, acceptance, rejection, cancellation, and proof) use Event Message semantics. `EncryptedLink::receive_private_application_messages` is the durable low-level receive API for mixed private message kinds; SDK/runtime code routes that ordered stream and then calls stateless parsers such as `parse_payment_request_event_message`, `parse_receipt_access_event_message`, and `parse_private_payment_list_json`. Do not add typed receive getters to `paykit-lib`; typed private message consumption belongs in the SDK/runtime layer.
