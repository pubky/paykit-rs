...
- Private payments are latest-state data: when multiple private payment envelopes are queued, `get_private_payments` returns the latest and supersedes older private-payment envelopes. Future event-like kinds such as receipts must preserve all messages in send order instead of using latest-wins semantics.
