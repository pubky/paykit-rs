...
{
    storage
        .transaction(|tx| Ok(tx.linked_peer(counterparty, counterparty_receiver_id)))
        .await
}
...
fn counterparty_receiver_id_for_operation(
    counterparty: &PubkyPublicKey,
    counterparty_receiver_id: Option<&PaykitReceiverId>,
    lease: Option<&PeerLinkOperationLease>,
) -> Result<PaykitReceiverId> {
    if let Some(lease) = lease {
        return Ok(lease.counterparty_receiver_id.clone());
    }

    counterparty_receiver_id.cloned().ok_or_else(|| {
        PaykitSdkError::Policy(format!(
            "counterparty {counterparty} receiver id is required for private link state"
        ))
    })
}
    ...
    S: StorageAdapter,
{
    storage
        .transaction(move |tx| {
            ...
            let counterparty_receiver_id = counterparty_receiver_id_for_operation(
                &counterparty,
                counterparty_receiver_id.as_ref(),
                lease.as_ref(),
            )?;
            if lease.is_none()
                && tx
    ...
    T: StorageTransaction + ?Sized,
{
    ...
    if let Some(link_state) = tx.encrypted_link_state(counterparty, counterparty_receiver_id) {
        tx.save_encrypted_link_state(EncryptedLinkStateRecord {
            counterparty: counterparty.clone(),
            counterparty_receiver_id: link_state.counterparty_receiver_id,
            link_snapshot: None,
    ...
    S: StorageAdapter,
{
    storage
        .transaction(move |tx| {
            ...
            mark_recovery_required_in_transaction(tx, &counterparty, &counterparty_receiver_id, now)
        })
        .await
}
    ...
    S: StorageAdapter,
{
    storage
        .transaction(move |tx| {
            ...
            let existing = tx.encrypted_link_state(&counterparty, &counterparty_receiver_id);
    ...
    storage
        .transaction(move |tx| {
            ...
            if let Some(existing) =
                tx.encrypted_link_state(&counterparty, &counterparty_receiver_id)
            {
    ...
    S: StorageAdapter,
{
    storage
        .transaction(move |tx| {
            ...
            let counterparty_receiver_id =
                counterparty_receiver_id_for_operation(&counterparty, None, lease.as_ref())?;
            let mut peer = tx
                .linked_peer(&counterparty, &counterparty_receiver_id)
                .unwrap_or_else(|| {
                    default_linked_peer(counterparty.clone(), counterparty_receiver_id.clone())
...
{
    storage
        .transaction(|tx| Ok(tx.encrypted_link_state(counterparty, counterparty_receiver_id)))
        .await
}
