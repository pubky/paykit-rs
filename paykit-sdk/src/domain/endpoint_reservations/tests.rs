...
async fn test_queue_private_payment_list_with_reservations_stores_linked_records() {
    ...
    let records = payment_endpoint_reservations(&storage, &counterparty, &receiver_id())
        .await
        .unwrap();
...
async fn test_queue_private_payment_list_with_reservations_preserves_existing_metadata() {
    ...
    assert_eq!(
        snapshot
            .payment_endpoint_reservations
            .get(&(counterparty.clone(), receiver_id(), "res-1".into()))
            .unwrap()
            .outbound_message_id,
...
async fn test_queue_private_payment_list_with_reservations_rejects_cancellation_claimed_id() {
    ...
    storage
        .transaction({
            ...
            move |tx| {
                let mut record = tx
                    .payment_endpoint_reservation(&counterparty, &receiver_id(), "res-1")
                    .unwrap();
...
async fn test_unattempted_superseded_reservation_cancellations() {
    ...
    let cancellations =
        unattempted_superseded_reservation_cancellations(&storage, &counterparty, &receiver_id())
            .await
            .unwrap();
